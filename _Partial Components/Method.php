...
/* FUNCTION FOR SHOWING TOTAL QUESTION OF SPECIFIC SUBJECT */
	...
	public function getSportNews(){
		$query ="SELECT * from Viw_News where Category = 'Sports' and status = 'Publish' order by News_ID DESC limit 4";
		mysqli_set_charset($this->db->link, 'UTF8');
		$getData = $this->db->select($query);
		return  $getData;
	}
	public function getAfghanistanNews(){
		$query ="SELECT * from Viw_News where Category = 'Afghanistan' and status = 'Publish' order by News_ID DESC limit 4";
		mysqli_set_charset($this->db->link, 'UTF8');
		$getData = $this->db->select($query);
		return  $getData;
	}
